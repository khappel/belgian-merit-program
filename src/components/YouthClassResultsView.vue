<template>
    <DataTable v-model:expandedRows="expandedRows" :value="Array.from(classData.values())" stripedRows
        ...
        <template #expansion="slotExpansion">
            <div class="p-1">
                <DataTable v-model:expandedRows="expandedHorseRows" :value="slotExpansion.data.youth" @rowExpand="onRowExpand"
                @rowCollapse="onRowCollapse" dataKey="exhibitor" :class="p-datatable-sm" tableStyle="min-width: 50rem"
...
export default {
    ...
    computed: {
        classData() {
            return new showViewData(store.youthShowData).ReturnYouthClassResults()
        },
